...
agg_kwargs = {
    ...
    f"{INDICATOR}_mean_total": (f"{INDICATOR}_mean", sum),
...
def get_full_ts(wah, monthly_los):
    ...
    full_ts = full_ts[[
        'RZ_ID', 'Year', 'Month', 'LoS', 'buffer', f'{INDICATOR}_total', f'{INDICATOR}_mean',
        'mean_anomaly_total', 'q50_anomaly_total',
        ...
        for ensemble in (sub_pbar:=tqdm(ENSEMBLES, leave=False)):
            ...
            if not os.path.exists(pathname_ts):
                ...
                if os.path.exists(basinpath):
                    ...
                    wah_buffer = wah_buffer[[
                        'RZ_ID', 'Year', 'Month', 'buffer', INDICATOR, f'{INDICATOR}_mean', f'{INDICATOR}_q50',
                        f'{INDICATOR}_q75', f'{INDICATOR}_q90'
                    ]].groupby(['RZ_ID', 'buffer', 'Year', 'Month']).agg(**agg_kwargs).reset_index()

                    # calculate accumulated anomaly metrics
                    wah_buffer['mean_anomaly_total'] = (
                        wah_buffer[f'{INDICATOR}_total'] - wah_buffer[f'{INDICATOR}_mean_total']
                    )
                    wah_buffer['q50_anomaly_total'] = (
                        wah_buffer[f'{INDICATOR}_total'] - wah_buffer[f'{INDICATOR}_q50_total']
                    )

                    # calculate accumulated deficit metrics
                    for stat in ['mean', 'q50', 'q75', 'q90']:
                        wah_buffer[f'{stat}_deficit_total'] = np.where(
                            wah_buffer[f'{INDICATOR}_total'] - wah_buffer[f'{INDICATOR}_{stat}_total'] < 0.0,
                            wah_buffer[f'{INDICATOR}_total'] - wah_buffer[f'{INDICATOR}_{stat}_total'],
                            0.0
                        )

                    # save
                    wah_buffer = wah_buffer[[
                        'RZ_ID', 'Year', 'Month', 'buffer', f'{INDICATOR}_total', f'{INDICATOR}_mean',
                        'mean_anomaly_total', 'q50_anomaly_total',
                        'mean_deficit_total', 'q50_deficit_total', 'q75_deficit_total', 'q90_deficit_total',
                    ]]
