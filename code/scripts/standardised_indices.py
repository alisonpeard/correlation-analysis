"""
Calculate standardised drought indices (SPI, SPEI).
...
def prepare_indices(df, tempdir, scenario, q=None, windows=[6, 12, 24]):
    ...
    for window in windows:
        ...
        df2 = fit_distribution(
            ...
            tempdir=os.path.join(tempdir, scenario.lower()), window=window, scenarios=[scenario],
        )
        df2['window'] = window
        distribution_dfs.append(df2)

    df2 = pd.concat(distribution_dfs)
    df2.to_csv(os.path.join(tempdir, scenario.lower(), 'parameters.csv'), index=False)
...
def calculate_indices(
    ...
    df_pars = pd.read_csv(os.path.join(tempdir, scenario.lower(), parameters_file))
    ...
    df4.to_parquet(os.path.join(tempdir, scenario.lower(), 'standardised_series.parquet'), index=False)
