"""
Aggregate WAH data to get time series per WRZ and buffer zone.

Steps:
    - Read all data (or at least all scenarios attempting)
    - Aggregate spatially
    - Calculate thresholds
    - Calculate SPI/SPEI (save parameters)
...
def aggregate_spatially(tempdir, df, buffered_extents, scenarios=['BS', 'NF', 'FF']):
    for scenario in (pbar := tqdm(scenarios)):
        ...
        output_path = os.path.join(tempdir, scenario.lower(), 'aggregated_series.parquet')
...
def main(config, scenarios=["BS", "NF", "FF"]):
    ...
    for scenario in scenarios:
        if not os.path.exists(os.path.join(tempdir, scenario.lower())):
            os.makedirs(os.path.join(tempdir, scenario.lower()))
